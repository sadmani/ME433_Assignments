...
int get_hex_array(char c){
    //Find character 'c' in the table provided for the OLED display and find associated hex values
    int hex_array = c - 0x20;
    return hex_array;
}
...
void oled_follow_accel(int acc_y, int acc_x){
    //Need to clear the LCD each time this is called, otherwise, we'll just keep adding to what was drawn before
    display_clear();
    //row and col define the origin of the axes/center of the oled screen
    int row = 32;
    int col = 64;

    //Draw a line proportional to the magnitude of the acceleration in the y or x direction. This involves scaling the values first
    int x_scaled;
    int y_scaled;
    x_scaled = acc_x*SCALE;
    y_scaled = acc_y*SCALE;

    //Before we proceed, need to know which direction to draw the line in based on the sign of the accelerations
    int x_dir;
    int y_dir;
    ...
    //Need the absolute value so that we decrease the magnitude as we draw the line in the while loop
    x_scaled = abs(x_scaled);
    y_scaled = abs(y_scaled);

    int width;

    //iterate through each of the values of x_scaled, which is what allows us to draw a line based on the magnitude
    while(x_scaled > 0){
        //correct for direction since we x_scaled is simply the magnitude
        row = row + 1*x_dir;
        //give the line thickness
        for(width = 0; width < 5; width++){              
           display_pixel_set(row, col+width, 1);
        };
        //after you draw the pixel, decrease the scaled value by 1- do this until scaled value goes to 0
        x_scaled = x_scaled - 1;
    }
    ...
    int height;
    while(y_scaled > 0){
        //correct for direction since we y_scaled is simply the magnitude
        col = col + 1*y_dir;
        //give the line thickness
        for(height = 0; height < 5; height++){
           display_pixel_set(row + height, col, 1); //set the pixel value
        };
        //after you draw the pixel, decrease the scaled value by 1- do this until scaled value goes to 0        
        y_scaled = y_scaled - 1;
    }
